...
function handleGetProducts($db, &$response) {
    $id       = isset($_GET['id']) ? (int)$_GET['id'] : 0;   // <— NEW
    ...
    if ($id > 0) {
        $sql = $baseSql . " AND p.ProductID = ? LIMIT 1";
        $stmt = $db->prepare($sql);
        $stmt->execute([$id]);
        ...
        $response['data'] = [
            'products' => $product,
            // categories are not strictly needed for the detail page; omit or include if you like:
            // 'categories' => []
        ];
        return;
    }

    // Otherwise, listing flow (category/search/paging)
    $sql = $baseSql;
    $params = [];
